Turbocoin Core version 0.12.0 is now available from:

  <https://turbocoin.us/bin/turbocoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Turbocoin-Qt (on Mac) or
turbocoind/turbocoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Turbocoin Core or other software:
...
pre-0.12 versions of Turbocoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Turbocoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Turbocoin transactions now use validation using
[libsecp256k1](https://github.com/turbocoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/turbocoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Turbocoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Turbocoin Core was to raise the default minimum
relay fee.

Turbocoin Core 0.12 will have a strict maximum size on the mempool. The
...
Turbocoin Core 0.12 also introduces new default policy limits on the length and
...
Turbocoin Core 0.12 nodes. Turbocoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/turbocoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Turbocoin Core 0.12 does not yet have support for
...
This allows running turbocoind without having to do any manual configuration.
...
Turbocoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Turbocoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Turbocoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Turbocoin Core 0.13, and it is at this time undecided whether the
...
Turbocoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Turbocoin Core automatically creates a hidden service to listen on, without
manual configuration. Turbocoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Turbocoin Core is listening, and
...
Turbocoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Turbocoin
Core to use floating fees. By default, Turbocoin Core will use floating
fees.
...
(default: `0.0002` TURBO/kB).

At all times, Turbocoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TURBO.
Furthermore, Turbocoin Core will never create transactions paying less than
...
`turbocoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/turbocoin/turbocoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/turbocoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/turbocoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /turbocoinrpc>
        ...
        # optional bypass turbocoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< turbocoinrpc:<password>
    </Location>
...
Restarting turbocoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Turbocoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new turbocoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include turbocoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix turbocoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of turbocoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [turbocoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libturbocoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "TURBO" (Ross Nicoll)
