Turbocoin Core version 0.13.1 is now available from:

  <https://turbocoin.us/bin/turbocoin-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a turbocoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Turbocoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/turbocoin/turbocoin/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Turbocoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Turbocoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Turbocoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of turbocoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of turbocoins being spent is exactly
...
- **Increased security for multisig:** Turbocoin addresses (both P2PKH addresses
  ...
  believed to be provided by Turbocoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Turbocoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Turbocoin's current ledger (called the UTXO set),
  ...
  note that Turbocoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Turbocoin users to individually opt-in to almost any change in the Turbocoin
  Script language when those users receive new transactions.  Features
  currently being researched by Turbocoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Turbocoin community members are studying several other improvements
...
[Segwit FAQ]: https://turbocoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://turbocoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/turbocoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/turbocoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/turbocoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/turbocoin/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://turbocoincore.org/en/2016/06/08/version-bits-miners-faq/
...
Since Turbocoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/turbocoin/bips/blob/master/bip-0147.mediawiki
...
- `turbocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `turbocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in turbocoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.turbocoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libturbocoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_turbocoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add turbocoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` turbocoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_turbocoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name turbocoin.qrc  (turbocoinsSG)
...
- #8817 `bcf3806` update turbocoin-tx to output witness data (jnewbery)
