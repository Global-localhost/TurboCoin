Turbocoin Core version 0.11.0 is now available from:

  <https://turbocoin.us/bin/turbocoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer TURBO/kB fee to be
...
For example, add the following to `turbocoin.conf`:
...
related to the blockchain in the turbocoin system: the raw blocks as received over
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Turbocoin Core to delete the raw block and undo data once
...
block index and UTXO databases. The minimum was chosen so that Turbocoin Core will
...
For an example script see [turbocoin-submittx](https://github.com/laanwj/turbocoin-submittx).
...
- #5334 `cf87536` libturbocoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new turbocoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only TurbocoinAmountField
- #5753 `0f44672` Add turbocoin logo to about screen
...
- #5809 `46bfbe7` Add turbocoin-cli man page
...
- #5849 `41259ca` contrib/init/turbocoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Turbocoin-Qt.app to Turbocoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for turbocoin-tx)
