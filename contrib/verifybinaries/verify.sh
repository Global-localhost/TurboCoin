#!/usr/bin/env bash
# Copyright (c) 2016 TurboCoin
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   turbocoincore.org and turbocoin.us and compares them.
...
WORKINGDIR="/tmp/turbocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://turbocoincore.org"
HOST2="https://turbocoin.us"
BASEDIR="/bin/"
VERSIONPREFIX="turbocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'turbocoin-',
...
if [ $? -ne 0 ]; then
   echo "turbocoin.us failed to provide signature file, but turbocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "turbocoin.us and turbocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Turbocoin Core binary release signing key installed?"
   fi
