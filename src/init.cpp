 ...
 */
static const char* BITCOIN_PID_FILENAME = "turbocoind.pid";
 ...
 * between turbocoind, and turbocoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/turbocoin/turbocoin>";
    const std::string URL_WEBSITE = "<https://turbocoincore.org>";
...
/** Sanity checks
 *  Ensure that Turbocoin is running in a usable environment with all
...
{
    // Make sure only a single Turbocoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if turbocoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if turbocoin is started while in a temporary directory.\n",
