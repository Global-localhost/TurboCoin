...
const char * const BITCOIN_CONF_FILENAME = "turbocoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "turbocoind -foo=bar
...
#else
    const char* pszModule = "turbocoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Turbocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Turbocoin
    // Mac: ~/Library/Application Support/Turbocoin
    // Unix: ~/.turbocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Turbocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Turbocoin";
#else
    // Unix
    return pathRet / ".turbocoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Turbocoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Turbocoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Turbocoin Core developers";
    }
