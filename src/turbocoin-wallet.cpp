...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s turbocoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        fprintf(stderr, "No method provided. Run `turbocoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
